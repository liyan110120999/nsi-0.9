...
//	    	上传机构logo图片	
		    ...
		    final int MAX_REQUEST_SIZE   = 1024 * 1024 * 50; // 50MB
		    	...
		    	String Id=request.getParameter("Id");
		    	 ...
		    	 System.out.println("机构api:图片上传，收到的ID:"+Id);
		    	 System.out.println("机构api:图片上传，----------------------------------------------------------------------");
		        ...
		        // 这个路径相对当前应用的目录
		        String uploadPath = "C:" + File.separator+"upImage"+ File.separator+"upInstitutionImg"+File.separator+Id;		     
		        // 如果目录不存在则创建
...
//		            @SuppressWarnings("unchecked")
		            ...
		            if (formItems != null && formItems.size() > 0) {
		                ...
		                for (FileItem item : formItems) {
		                    // 处理不在表单中的字段
		                    if (!item.isFormField()) {	                    	
		                        String filePath = uploadPath + File.separator + Id+"-logo.jpg";
		                        File storeFile = new File(filePath);                   
		                        //	                        上传图片地址至数据库
		                        String sql="UPDATE nsi_institution_data SET Institution_logo='/upImage/upInstitutionImg/"+Id+"/"+Id+"-logo.jpg' WHERE Id='"+Id+"' ";    			
		        				DB.alter(sql);		                        
		                        // 在控制台输出文件的上传路径                     
