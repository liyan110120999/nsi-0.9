...
// 			jq Ajax
 			var pass_insert= function(aa,bb){			    	
 				$.ajax({
 				    type : "get",
 				    async:false,
 				    url : ApiUrl.address+"/School_api?whereFrom=verify_insert",	
 				    data: {"School_ID":aa},
...
//  				    	触发积分模态框
	 			...
	 			var No_insert= function(aa){			    	
	 				$.ajax({
	 				    type : "get",
	 				    async:false,
	 				    url : ApiUrl.address+"/School_api?whereFrom=verify_No_insert",	
	 				    data: {"School_ID":aa},
	 				    dataType : "jsonp",//数据类型为jsonp  
	 				    jsonp: "Callback",//服务端用于接收callback调用的function名的参数  
	 				    success : function(data){
 			...
 			var pass_alter= function(aa,bb){			    		 				
 				$.ajax({
 				    type : "get",
 				    async:false,
 				    url : ApiUrl.address+"/School_api?whereFrom=verify_alter",	
 				    data: {"School_ID":aa},
 				    dataType : "jsonp",//数据类型为jsonp  
 				    jsonp: "Callback",//服务端用于接收callback调用的function名的参数  
 				    success : function(data){
...
//  				    	window.location.reload();
			    ...
			    var No_alter= function(aa){			    		 				
	 				$.ajax({
	 				    type : "get",
	 				    async:false,
	 				    url : ApiUrl.address+"/School_api?whereFrom=verify_No_alter",	
	 				    data: {"School_ID":aa},
	 				    dataType : "jsonp",//数据类型为jsonp  
	 				    jsonp: "Callback",//服务端用于接收callback调用的function名的参数  
	 				    success : function(data){
...
<!-- 	新增审核 -->
	...
	//执行查询  
	ResultSet rs = stmt.executeQuery("select * from nsi_school_data_verify where VerifySign=11 order by load_time DESC limit 0,10");  
	%>  
	
	<table class="table">
	<caption><h3>学校新增-审核</h3></caption>
	<thead>
		<tr>
			<th>临时ID</th>
			<th>学校名(英文名)</th>
	...
	{%>  
		...
		<tr class="active">
			...
			<td><%=rs.getString("School_name")+rs.getString("School_EnglishName")%></td>
			<td><%=rs.getString("School_properties")%></td>
...
<!-- 	修改审核-->
	...
	//执行查询  
	ResultSet rs02 = stmt02.executeQuery("select * from nsi_school_data_verify where VerifySign=12 order by load_time DESC limit 0,10");  
	%>  
	
	<table class="table">
	<caption><h3>学校修改-审核</h3></caption>
	<thead>
		<tr>
			<th>临时ID</th>
			<th>学校名(英文名)</th>
	...
	{%>  
		...
		<tr class="active">
			...
			<td><%=rs02.getString("School_name")+rs02.getString("School_EnglishName")%></td>
			<td><%=rs02.getString("School_properties")%></td>
